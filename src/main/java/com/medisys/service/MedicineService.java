...
public class MedicineService {
    ...
    /**
     * Same as getCatalog, sorted.
     *
     * @param sort "price-asc", "price-desc", or anything else for A to Z
     */
    public List<Medicine> getCatalog(String keyword, Integer categoryId, String sort) throws SQLException {
        List<Medicine> medicines = getCatalog(keyword, categoryId);   // already A to Z
    ...
    public List<Medicine> getRelated(Medicine medicine) throws SQLException {
        List<Medicine> related = new ArrayList<>();
        for (Medicine m : getCatalog(null, medicine.getCategoryId())) {
            if (m.getId() != medicine.getId()) {
                related.add(m);
            }
        }
        related.sort(Comparator.comparing(Medicine::isOutOfStock));   // false (in stock) first
